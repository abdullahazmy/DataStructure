...
void doubleLinkedList::nbour(int k)
{
    Node *cur = get_nth(k);
...
void doubleLinkedList::swap_forward_back(int k)
{
    if (k > length)
        return;
    Node *left = get_nth(k);
    Node *right = get_nth(length - k + 1);
    if (left->next == right || k == 1 || k == length)
        nbour(k);
    else
    {
